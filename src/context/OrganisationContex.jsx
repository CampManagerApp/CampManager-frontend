...
export default function OrganisationProvider(props) {
    ...
    async function delete_campaign_counsellors(org_id, campaing_id, counsellors) {
        try {
            for (const counsellor of counsellors) {
                await delete_org_campaign_counsellor(org_id, campaing_id, counsellor.fullName)
            }
        } catch (error) {
            if (!error.response) {
                showErrorMessage(t("ERRORS.CONEXION_ERROR.ERROR_MODAL_TITLE"), t("ERRORS.CONEXION_ERROR.ERROR_MODAL_BODY"))
    ...
    async function update_campaign(org_id, campaign_name, new_campaign_name, start, end) {
        try {
            await update_org_campaign(org_id, campaign_name, new_campaign_name, start, end)
        } catch (error) {
            if (!error.response) {
                showErrorMessage(t("ERRORS.CONEXION_ERROR.ERROR_MODAL_TITLE"), t("ERRORS.CONEXION_ERROR.ERROR_MODAL_BODY"))
            } else if (error.response.status == 404) {
    ...
    async function solve_campaign_table(org_id, camp_id, table_name) {
        try {
            const data = await solve_org_campaign_table(org_id, camp_id, table_name)
            return data
        } catch (error) {
            if (!error.response) {
                showErrorMessage(t("ERRORS.CONEXION_ERROR.ERROR_MODAL_TITLE"), t("ERRORS.CONEXION_ERROR.ERROR_MODAL_BODY"))
    ...
    async function get_campaign_table(org_id, camp_id, table_name) {
        try {
            const data = await get_org_campaign_table(org_id, camp_id, table_name)
            return data
        } catch (error) {
            if (!error.response) {
                showErrorMessage(t("ERRORS.CONEXION_ERROR.ERROR_MODAL_TITLE"), t("ERRORS.CONEXION_ERROR.ERROR_MODAL_BODY"))
